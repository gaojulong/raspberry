...
//       return password;   //解码一下
     ...
     *保存IP
     */
    public void loadUserIp() {
        if (!getAccount().equals("") || !getAccount().equals("请输入IP以及端口")) {
            SharedPreferencesUtils helper = new SharedPreferencesUtils(this, "setting");
            helper.putValues(new SharedPreferencesUtils.ContentValue("ip", getIP()));
        }
    ...
    public String getIP(){
        return et_ip.getText().toString().trim();//去掉空格
    }
    /**
     * 获取账号
     */
